  <template>
    <div class="app-container">
      <el-button type="primary" class="add-button" @click="handleAdd" icon="el-icon-plus">Add New Room</el-button>
      <el-button type="primary" class="report-button" @click="fetchMaintenanceReports">
        Messages
      </el-button>
      <el-dropdown @command="handleDownloadReport">
        <el-button type="primary" class="report-button" style="margin-left: 10px;">
          Generate Room Usage Report<i class="el-icon-arrow-down el-icon--right"></i>
        </el-button>
        <el-dropdown-menu slot="dropdown">
          <el-dropdown-item command="1">Last 1 Month</el-dropdown-item>
          <el-dropdown-item command="2">Last 2 Months</el-dropdown-item>
          <el-dropdown-item command="3">Last 3 Months</el-dropdown-item>
        </el-dropdown-menu>
      </el-dropdown>
      <el-table :data="classrooms" style="width: 1490px; margin-top: 20px;" border>
      ...
      <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="500px">
        <el-form :model="currentClassroom" label-width="100px" :rules="classroomRules" ref="classroomForm">
          <el-form-item label="Room ID">
            <el-input v-model="currentClassroom.id" type="number" disabled />
          </el-form-item>

          <el-form-item label="RoomName" prop="roomName" required>
          ...
          <el-form-item label="Room Type" prop="roomType" required>
            <el-select v-model="currentClassroom.roomType" placeholder="Select Room Type">
              <el-option label="Teaching Room" :value="1" />
              <el-option label="Meeting Room" :value="2" />
              <el-option label="Activity Room" :value="3" />
            </el-select>
          </el-form-item>

          <el-form-item label="Capacity" prop="capacity" required>
            <el-input v-model.number="currentClassroom.capacity" type="number" />
          </el-form-item>
      ...
      <el-dialog title="Available Maintenance Time Ranges" :visible.sync="showFreeTimeDialog" width="500px">
        <el-timeline>
          <el-timeline-item
            v-for="(range, index) in freeTimeRanges"
            :key="index"
            :timestamp="`${formatDate(range.start)} ~ ${formatDate(range.end)}`"
            type="info"
          >
            Time Slot {{ index + 1 }}
          </el-timeline-item>
      ...
      <el-dialog title="Maintenance Management" :visible.sync="maintenanceDialogVisible" width="800px">
        ...
        <el-button type="text" @click="showFreeTimeDialog = true" style="margin-bottom: 10px;">
          View Available Maintenance Time Ranges
        </el-button>
        <el-form label-width="100px" style="margin-top: 20px;">
          ...
          <el-form-item label="End Time">
            <el-date-picker
                v-model="newMaintenance.enddate"
                type="date"
                placeholder="Select date"
                style="width: 150px"
              />
              <el-time-select
                v-model="newMaintenance.endtime"
        ...
        <div slot="footer">
          ...
          <el-button type="primary" @click="addMaintenanceRecord">Add Maintenance and reassign</el-button>
        </div>
      </el-dialog>
  ...
  <script>
  import { getRooms, getLecturers ,updateRoomPermission, getPermissionUser, deleteRoom, addRoom, updateRoom, getApprovalResources,approveApprovalResource, rejectRestrictedResource, getNameByUid, getMaintenance, addMaintenance, deleteMaintenance, getMaintenanceReports, readMessage, getRoomUtilization, getFreeMaintainTime} from '@/api/room'
  export default {
    watch: {
      'newMaintenance.date'(val) {
        if (!this.newMaintenance.enddate || this.newMaintenance.enddate !== val) {
          this.newMaintenance.enddate = val;
        }
      },
      'newMaintenance.enddate'(val) {
        if (!this.newMaintenance.date || this.newMaintenance.date !== val) {
          this.newMaintenance.date = val;
        }
      }
    },
    data() {
      return {
        classroomRules: {
          roomName: [
            { required: true, message: 'Room name is required', trigger: 'blur' }
          ],
          location: [
            { required: true, message: 'Location is required', trigger: 'blur' }
          ],
          roomType: [
            { required: true, message: 'Room type is required', trigger: 'change' }
          ],
          capacity: [
            { required: true, message: 'Capacity is required', trigger: 'blur' },
            { type: 'number', message: 'Capacity must be a number', trigger: 'blur' }
          ]
        },
        approvalOrders: [],
        showFreeTimeDialog: false,
        userCache: {},
        classrooms: [],
    ...
    methods: {
      ...
      getFreeTime(roomId){
        getFreeMaintainTime(roomId)
        .then(response => {
          const rawRanges = response.data.data;
          const merged = this.mergeTimeRanges(rawRanges);
          this.freeTimeRanges = merged;
        })
        .catch(error => {
          console.error('Error fetching free maintenance time:', error);
          this.$message.error('Failed to get free maintenance time.');
        });
      },
      handleDownloadReport(months) {
        this.downloadLoading = true;
        getRoomUtilization(months).then(response => {
          ...
          import('@/vendor/Export2Excel').then(excel => {
            const tHeader = [
              'Room Name',
              'Cancel Time', 'Cancellation Rate (%)',
              'Maintenance Time (h)', 'Maintenance Duration (h)',
            ...
            const filterVal = [
              'roomName',
              'cancelTime', 'cancellationRate',
              'maintenanceTime', 'maintenanceDuration',
              'CancellationReasons',
              'Utilization',
              'Suggestion'
            ];

            const data = this.formatReportJson(filterVal, rawData);

            excel.export_json_to_excel({
              header: tHeader,
              data,
              filename: `room-usage-report-${months}month`
            });
      ...
      formatReportJson(filterVal, jsonData) {
        return jsonData.map(item => {
          return filterVal.map(key => {
            switch (key) {
              case 'CancellationReasons': {
                const cr = item.CancellationReasons || {};
                return `Schedule Change: ${cr["Schedule Change"] ?? 0}%\n` +
                      `User Request: ${cr["User Request"] ?? 0}%\n` +
                      `Mistake: ${cr["Mistake"] ?? 0}%\n` +
                      `Equipment Failure: ${cr["Equipment Failure"] ?? 0}%\n` +
                      `Other: ${cr["Other"] ?? 0}%`;
              }

              case 'Utilization': {
                const u = item.Utilization || {};
                return `Class: ${u.classUtilization?.toFixed(2) ?? '0'}%\n` +
                      `Booking: ${u.bookingUtilization?.toFixed(2) ?? '0'}%\n` +
                      `Total: ${u.totalUtilization?.toFixed(2) ?? '0'}%`;
              }
              ...
              case 'Suggestion': {
                ...
                if (s["Increase Utilization"]) {
                  lines.push("Increase Utilization:");
                  lines.push(s["Increase Utilization"]);
                  lines.push("");
                }

                // 2. Reduce Cancellations
                if (s["Reduce Cancellations"]) {
                  const rc = s["Reduce Cancellations"];
                  const reasons = ["Schedule Change", "Equipment Failure", "User Mistake", "Emergency", "Other", "Summary"];
                  const reasonLines = reasons
                    .filter(reason => rc[reason])
                    .map(reason => `- ${reason}: ${rc[reason]}`);

                  if (reasonLines.length > 0) {
                    lines.push("Reduce Cancellations:");
                ...
                if (s["Reduce Maintenance Time"]) {
                  lines.push("Reduce Maintenance Time:");
                  lines.push(s["Reduce Maintenance Time"]);
                }
      ...
      handleMaintenance(row) {
        ...
        this.sendMaintenance.roomId = this.currentRoomId;
        this.newMaintenance.roomId = this.currentRoomId;
        this.getMaintenanceList();
        this.getFreeTime(row.id);
        this.maintenanceDialogVisible = true;
      },
      getMaintenanceList(){
        getMaintenance({roomId: this.currentRoomId,
      ...
      if (!start || !end || !this.newMaintenance.reason) {
        this.$message.error('Please fill in all maintenance details.');
        return;
      }
      this.sendMaintenance.startTime = this.formatDateToISO(this.combinedStartTime)
      this.sendMaintenance.endTime = this.formatDateToISO(this.combinedEndTime)
      this.sendMaintenance.reason = this.newMaintenance.reason
        addMaintenance(this.sendMaintenance)
        .then((response) => {
          if (response.data.code === 500) {
            this.$message.error('The selected time range is invalid.');
          }else{
          this.$message.success('Maintenance record added successfully!');
          this.fetchClassrooms();
          this.getMaintenanceList();
          this.getFreeTime(this.currentRoomId);
          this.sendMaintenance = { startTime: '', endTime: '', reason: '' };
          this.newMaintenance = {date: '',enddate: '',time:'',endtime:'',reason: ''}
          }
        }).catch(error => {
    ...
    computed: {
      combinedStartTime() {
        if (this.newMaintenance.date && this.newMaintenance.time) {
          return new Date(`${this.formatDateToYYYYMMDD(this.newMaintenance.date)} ${this.newMaintenance.time}`);
        }
        return null;
      },
      combinedEndTime() {
        if (this.newMaintenance.enddate && this.newMaintenance.endtime) {
          return new Date(`${this.formatDateToYYYYMMDD(this.newMaintenance.enddate)} ${this.newMaintenance.endtime}`);
        }
        return null;
      },
      endTimeOptions() {
        if (!this.newMaintenance.time) {
          return {
            start: '08:30',
            step: '00:30',
            end: '22:00'
        ...
        const [hourStr, minuteStr] = this.newMaintenance.time.split(':');
      ...
      startTimeOptions() {
        if (!this.newMaintenance.endtime) {
          return {
            start: '08:00',
            step: '00:30',
            end: '21:30'
        ...
        const [hourStr, minuteStr] = this.newMaintenance.endtime.split(':');
        let hour = parseInt(hourStr, 10);
        let minute = parseInt(minuteStr, 10);
